...
# 而所有网络层检查都显示"正常",极难定位。
# 该账号只允许【一个设备在线】(online_device_total=1),而这个位子被
# 上游的小米路由器 WAN(10.134.138.11)占着 —— eno2 直连校园网就是"第二个设备",
# 会话建不起来,于是被劫持。给 eno2 认证成功只会把路由器顶下线,连带干掉 eno1
# 和整个 LAN,所以 eno2 作为备份在结构上不可能成立。
#
